        ...
        if os.path.exists(pasta_alvo):
            ...
            return (
                jsonify(
                    {
                        "status": "sucesso",
                        "mensagem": f"A pasta '{pasta_alvo}' foi limpa com sucesso.",
                    }
                ),
                200,
            )
            ...
            print(
                f"INFO: Pasta '{pasta_alvo}' não encontrada, nenhuma ação foi necessária."
            )
            return (
                jsonify(
                    {
                        "status": "info",
                        "mensagem": f"A pasta '{pasta_alvo}' não existia.",
                    }
                ),
                200,
            )
