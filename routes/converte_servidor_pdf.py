...
from utils.formata_datas import (
    data_atual,
    pega_final_de_semana,
    pega_quantidade_dias_mes,
)
...
def formatar_horario_para_hh_mm_v2(valor_horario):
    ...
    if isinstance(valor_horario, timedelta):
        ...
        hours = (
            total_seconds // 3600
        ) % 24  # Garante que as horas fiquem dentro de 0-23
        ...
        try:
            ...
            # Depois como HH:MM
                ...
                return dt_obj.strftime(
                    "%H:%M"
                )  # Já está no formato, mas re-formata para garantir
            else:
...
def converte_servidor_pdf():
    try:
        ...
        for funcionario in funcionarios:
            ...
            feriados_do_mes, pontos_facultativos_mes = pegar_feriados_mes(
                ano, mes_numerico, estado=estado_funcionarios
            )
            ...
            troca_de_dados = {
                ...
                "CAMPO ANO": str(ano),
                "CAMPO HORARIO": funcionario.get(
                    "horario", ""
                ),  # Mantido como está, sem formatação específica aqui
                "CAMPO ENTRADA": formatar_horario_para_hh_mm_v2(
                    funcionario.get("horarioentrada", "")
                ),
                "CAMPO SAÍDA": formatar_horario_para_hh_mm_v2(
                    funcionario.get("horariosaida", "")
                ),
            ...
            caminho_pasta = (
                f"setor/{setor_limpo}/servidor/{mes_por_extenso}/{nome_limpo}"
            )
            os.makedirs(caminho_pasta, exist_ok=True)

            nome_base = f"{nome_limpo}_FREQUENCIA"
            docx_path = os.path.abspath(
                os.path.join(caminho_pasta, f"{nome_base}.docx")
            )
...
def cria_dias_da_celula(
    ...
    while len(table.rows) > target_total_rows_in_table:
        ...
        print(
            f"INFO: Linha excedente removida. Total de linhas agora: {len(table.rows)}"
        )
    ...
    while len(table.rows) < target_total_rows_in_table:
        ...
        print(
            f"INFO: Linha faltante adicionada. Total de linhas agora: {len(table.rows)}"
        )
    ...
    for i in range(quantidade_dias_no_mes):
        ...
        data_atual = date(
            ano, mes_numerico, dia
        )  # Use o nome data_atual como no seu código
        dia_semana = pega_final_de_semana(
            ano, mes_numerico, dia
        )  # Assume que esta função existe
        ...
        dia_paragraph = (
            dia_cell.paragraphs[0] if dia_cell.paragraphs else dia_cell.add_paragraph()
        )
        dia_paragraph.alignment = (
            WD_PARAGRAPH_ALIGNMENT.CENTER
        )  # Reafirma o alinhamento se for um novo parágrafo
        ...
        if texto_status:  # Escreve SÁBADO ou DOMINGO
            ...
            for j in [2, 5, 9, 13]:  # Seus índices de coluna originais
                if j < len(row.cells):
                    cell = row.cells[j]
                    cell.text = (
                        texto_status  # Define o texto, limpando parágrafos anteriores
                    )
                    ...
                    for paragraph in cell.paragraphs:
                        ...
                        for (
                            run
                        ) in paragraph.runs:  # O texto agora está em um ou mais runs
                            run.font.bold = True
        ...
        if data_atual in pontos_facultativos and dia_semana not in [5, 6]:
            ...
            for j in [2, 5, 9, 13]:
                if j < len(row.cells):
                    ...
                    print(
                        f"AVISO: Índice de coluna {j} para PONTO FACULTATIVO fora dos limites."
                    )
        ...
        if funcionario.get("feriasinicio") and funcionario.get("feriasfinal"):
            ...
            ferias_inicio = (
                ferias_inicio_raw.date()
                if hasattr(ferias_inicio_raw, "date")
                else ferias_inicio_raw
            )
            ferias_final = (
                ferias_final_raw.date()
                if hasattr(ferias_final_raw, "date")
                else ferias_final_raw
            )

            if (
                isinstance(ferias_inicio, date)
                and isinstance(ferias_final, date)
                and (
                    ferias_inicio <= data_atual <= ferias_final
                    and dia_semana not in [5, 6]
                )
            ):
                set_row_background(row, "C5E0B4")  # VERDE
                for j in [2, 5, 9, 13]:
                    if j < len(row.cells):
                        ...
                        print(
                            f"AVISO: Índice de coluna {j} para FÉRIAS fora dos limites."
                        )
