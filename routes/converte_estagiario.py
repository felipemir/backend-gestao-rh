...
def converte_estagiario_pdf():
    try:
        ...
        feriados_proximo_mes, pontos_fac_proximo_mes = pegar_feriados_mes(
            ano_proximo_mes_periodo,
            mes_numerico_proximo_periodo,
            estado=estado_para_feriados,
        )
        ...
        for estagiario in estagiarios:
            ...
            caminho_pasta = (
                f"setor/{setor_limpo}/estagiario/{mes_por_extenso}/{nome_limpo}"
            )
...
def cria_dias_da_celula(
    doc, ano, mes_numerico, estagiario, feriados, pontos_facultativos
):
    ...
    for i, dia_info in enumerate(dias_periodo):
        ...
        if estagiario.get("feriasinicio") and estagiario.get("feriasfinal"):
            ferias_inicio = (
                estagiario["feriasinicio"].date()
                if hasattr(estagiario["feriasinicio"], "date")
                else estagiario["feriasinicio"]
            )
            ferias_final = (
                estagiario["feriasfinal"].date()
                if hasattr(estagiario["feriasfinal"], "date")
                else estagiario["feriasfinal"]
            )
        ...
        if dia_semana == 5:
            ...
            texto = "DOMINGO"
        elif is_ponto_facultativo and dia_semana not in [
            5,
            6,
