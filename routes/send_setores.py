...
def download_zip(setor, mes):
    try:
        ...
        if not result:
            print(
                "RESULTADO DA CONSULTA: Arquivo ZIP não encontrado no banco de dados."
            )  # Log específico
            return (
                jsonify(
                    {
                        "erro": "Arquivo ZIP não encontrado no banco de dados para os critérios fornecidos"
                    }
                ),
                404,
            )
        ...
        zip_path_verified = os.path.normpath(
            zip_path_from_db
        )  # Use o caminho EXATO do DB

        print(f"Caminho do arquivo no DB: '{zip_path_from_db}'")
        print(
            f"Tentando acessar arquivo ZIP no caminho verificado: '{zip_path_verified}'"
        )

        if not os.path.exists(zip_path_verified):
            print(
                f"ERRO FÍSICO: Arquivo não encontrado em '{zip_path_verified}'"
            )  # Log específico
            return (
                jsonify(
                    {
