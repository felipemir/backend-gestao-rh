    ...
    try:
        ...
        for setor_nome in setores:
            ...
            for funcionario in funcionarios:
                ...
                feriados_do_mes, pontos_facultativos_do_mes = pegar_feriados_mes(
                    ano, mes_numerico, estado=estado_funcionario
                )

                # CORREÇÃO 1: A linha de print foi atualizada para usar as variáveis corretas
                print(
                    f"DEBUG: Para {funcionario.get('nome')}, Feriados: {feriados_do_mes}, Pontos Facultativos: {pontos_facultativos_do_mes}"
                )
                ...
                nome_limpo = (
                    funcionario.get("nome", "NOME_PADRAO").strip().replace("/", "_")
                )
                ...
                pdf_path = os.path.abspath(
                    os.path.join(caminho_pasta, f"{nome_base}.pdf")
                )
                doc.save(docx_path)
        ...
        if not arquivos_zip_gerados_todos_setores:
            return (
                jsonify({"message": "Nenhum arquivo ZIP de funcionários foi gerado."}),
                200,
            )
        ...
        if len(arquivos_zip_gerados_todos_setores) > 1:
            ...
            cursor.execute(
                ...
                (
                    "multissetores_funcionarios",
                    mes_por_extenso_geral,
                    str(ano),
                    zip_final_path,
                    "multissetores_funcionarios_geral",
                ),
            )
            conexao_principal.commit()
            if conexao_principal and conexao_principal.is_connected():
            ...
            return send_file(
                arquivos_zip_gerados_todos_setores[0],
                mimetype="application/zip",
                as_attachment=True,
                download_name=os.path.basename(arquivos_zip_gerados_todos_setores[0]),
            )
    ...
    for i in range(quantidade_dias_no_mes):
        ...
        if funcionario.get("feriasinicio") and funcionario.get("feriasfinal"):
            ...
            if (
                ...
                and (
                    ferias_inicio <= data_atual_obj <= ferias_final
