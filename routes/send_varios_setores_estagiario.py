...
bp_send_varios_setores_estagiarios_pdf = Blueprint(
    "bp_send_varios_setores_estagiarios_pdf", __name__
)


@bp_send_varios_setores_estagiarios_pdf.route(
    "/api/setores/estagiarios/pdf/download-zip-multiestagiarios/<mes>", methods=["GET"]
)
def download_zip_multiestagiarios_estagiarios(mes):  # Função renomeada
    try:
        ...
        if not result:
            ...
            return (
                jsonify(
                    {"erro": "Arquivo ZIP multi-setores de estagiários não encontrado"}
                ),
                404,
            )
        ...
        print(
            f"Tentando acessar arquivo ZIP (multiestagiarios) no caminho: {zip_path_verified}"
        )
        ...
        download_name = os.path.basename(
            zip_path_verified
        )  # Usa o nome real do arquivo do caminho
        ...
        return (
            jsonify(
                {"erro": f"Erro ao baixar ZIP multi-setores de estagiários: {str(e)}"}
            ),
            500,
        )
