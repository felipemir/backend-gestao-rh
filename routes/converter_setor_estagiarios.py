...
bp_converte_setor_estagiario_pdf = Blueprint(
    "bp_converte_setor_estagiario_pdf", __name__
)
    ...
    if isinstance(valor_horario, str):
        try:
            if valor_horario.count(":") == 2:
                return datetime.strptime(valor_horario, "%H:%M:%S").strftime("%H:%M")
            elif valor_horario.count(":") == 1:
                return datetime.strptime(valor_horario, "%H:%M").strftime("%H:%M")
...
def converte_setores_estagiarios_pdf():
    ...
    try:
        ...
        if not setores_nomes or not isinstance(setores_nomes, list):
            return (
                jsonify({"erro": "Nenhum setor selecionado ou formato inválido"}),
                400,
            )
        ...
        feriados_corrente, pontos_fac_corrente = pegar_feriados_mes(
        ...
        if mes_proximo > 12:
            mes_proximo = 1
            ano_proximo += 1
        feriados_proximo, pontos_fac_proximo = pegar_feriados_mes(
            ano_proximo, mes_proximo, estado=estado_para_feriados
        )

        todos_feriados_do_periodo = list(set(feriados_corrente + feriados_proximo))
        todos_pontos_facultativos_do_periodo = list(
            set(pontos_fac_corrente + pontos_fac_proximo)
        )
        ...
        for setor_nome in setores_nomes:
            ...
            caminho_pasta_base_setor = (
                f"setor/estagiarios/{setor_limpo}/{mes_por_extenso}"
            )
            ...
            for estagiario in estagiarios:
                ...
                cria_dias_da_celula(
                    ...
                    mes_numerico,
                    estagiario,
                    todos_feriados_do_periodo,
                    todos_pontos_facultativos_do_periodo,
                )
                ...
                troca_de_dados = {
                    ...
                    "CAMPO ENTRADA": formatar_horario_para_hh_mm_v2(
                        estagiario.get("horario_entrada", "")
                    ),
                    "CAMPO SAÍDA": formatar_horario_para_hh_mm_v2(
                        estagiario.get("horario_saida", "")
                    ),
                ...
                nome_limpo = (
                    estagiario.get("nome", "NOME_PADRAO").strip().replace("/", "_")
                )
                nome_base = f"FREQUENCIA_ESTAGIARIO_{nome_limpo.replace(' ', '_')}"
                docx_path = os.path.abspath(
                    os.path.join(caminho_pasta_base_setor, f"{nome_base}.docx")
                )
                pdf_path = os.path.abspath(
                    os.path.join(caminho_pasta_base_setor, f"{nome_base}.pdf")
                )
                doc.save(docx_path)
        ...
        if not arquivos_zip_dos_setores:
            return (
                jsonify({"message": "Nenhum arquivo ZIP de estagiários foi gerado."}),
                200,
            )
        ...
        if len(arquivos_zip_dos_setores) > 1:
            ...
            return send_file(
                ...
                mimetype="application/zip",
                as_attachment=True,
                download_name=os.path.basename(arquivos_zip_dos_setores[0]),
            )

        return (
            jsonify(
                {"message": "Processamento concluído, mas nenhum ZIP para enviar."}
            ),
            200,
        )
        ...
        return (
            jsonify(
                {"erro": f"Erro ao processar setores de estagiários: {str(exception)}"}
            ),
            500,
        )
...
def cria_dias_da_celula(
    doc, ano_param, mes_param, estagiario, feriados, pontos_facultativos
):
    """
        ...
        if text_to_write is None:
            if estagiario.get("feriasinicio") and estagiario.get("feriasfinal"):
                ...
                if (
                    ...
                    and (ferias_inicio <= current_date_obj <= ferias_final)
                ):
